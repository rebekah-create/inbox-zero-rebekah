...
- **D-02 — Default end = start + 60min when missing.** When the Haiku extractor returns a `start` but no `end`, treat the candidate as a 60-min interval for the overlap check **only**. Do NOT persist this default end to the new Google Calendar event — pass `null` end through and let `createCalendarEvent` apply its own duration default (or write start-only if Google permits). The 60-min default is reconciliation-internal.
